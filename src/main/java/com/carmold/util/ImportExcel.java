...
public class ImportExcel {
     ...
     * @throws IOException
     */
    public static HashMap<String, ArrayList> analysisFile(MultipartFile file, String fileJsonPath) throws IOException {
        HashMap<String, ArrayList> hashMap = new HashMap<>();

        String path = ReadJson.class.getClassLoader().getResource(fileJsonPath).getPath();//获取文件路径
        String s = ReadJson.readJsonFile(path);
        JSONObject jobj = JSON.parseObject(s); //读取json文件
        ...
        if(workbook != null){
            ...
            for(int sheetNum = 0;sheetNum < workbook.getNumberOfSheets();sheetNum++){
                ...
                for(int cellNum = sheet.getFirstRowNum(); cellNum < sheet.getRow(0).getLastCellNum(); cellNum++){
                    Cell cell = sheet.getRow(0).getCell(cellNum);//读取表中第一行
                    for (int i = 0 ; i < data.size();i++) {//表头存储
                        JSONObject json = (JSONObject) data.get(i);
                        String key = (String)json.get(getCellValueByCell(cell));
...
//                    String[] strings = new String[lastCellNum2];
                    ...
                    for(int cellNum = firstCellNum; cellNum < lastCellNum; cellNum++){
                        ...
                        if( getCellValueByCell(cell) != "" && list.get(cellNum) == null ){
                            arrayList.clear();
                            //将错误字段放入数组中
                        ...
                        if(list.get(cellNum) != null){//判断表头是否为空
                            //按表头的顺序添加当前行的数据
                            language.put(list.get(cellNum).toString() , getCellValueByCell(cell));
                        }
                    }
    ...
    public static String getCellValueByCell(Cell cell) {
        ...
        switch (cellType) {
            case Cell.CELL_TYPE_NUMERIC: // 数字
                ...
                if (DateUtil.isCellDateFormatted(cell) || format!=0) {
